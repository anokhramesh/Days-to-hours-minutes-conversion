...
def days_to_units(num_of_days,conversion_unit):#function for conversion.
    if conversion_unit == "hours": # if user input is nimber of days and hours-apply this formula.
        return f"{num_of_days} days are {num_of_days * 24} hours"
    elif conversion_unit =="minutes": # if user input is number of days and minute-apply this formula. 
        return f"{num_of_days} days are {num_of_days *24*60} minutes" #return the calculated value.
    else:
...
def validate_and_execute(): #function for validating the calculation.
    try:
        user_input_number =int(days_and_unit_dictionary["days"]) # type casting(convert the user inputed string to integer and store it in to variable-user_input_number)
        if user_input_number > 0: #only convert if user input is a positive value.
            calculated_value = days_to_units(user_input_number,days_and_unit_dictionary["unit"]) #saving the values to calculated_value variable.
    ...
    except ValueError:#if user input is a float value or string or character accept the valueError -print the belo message.
...
while user_input != "exit":#continue loop untill the user typed a string -exit.
    ...
    # clickon the Run button.
    # Enter the Days and unit asper the format 5:hours or 1:minutes.(convert 5 days to hours)
    # python will display- 5 days are 120 hours.
